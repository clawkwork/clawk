...
func TestTime(t *testing.T) {
	...
	require.Equal(t, now.UnixNano(), now2.UnixNano(), "%s != %s", now, now2)
}

func TestLargeFile(t *testing.T) {
	testFiles := []testFile{
		{Path: "small", File: &File{}, DataSize: mib},        // can't change type
		{Path: "medium", File: &File{}, DataSize: 200 * mib}, // can't change type
		{Path: "large", File: &File{}, DataSize: 600 * mib},  // can't change type
	}
	runTestsOnFiles(t, testFiles)
}
