...
// gib replaces upstream's internal/memory dependency.
const gib = 1 << 30
...
const (
	...
	defaultMaxDiskSize = 16 * gib                       // 16GB
	...
	maxFileSize             = 128 * gib // 128GB file size maximum for now
...
// findPath resolves p relative to root, following symlinks in every
// component the way the kernel would. This matters for adding files on
// top of merged-usr images (Debian, Ubuntu and derivatives), where
// /sbin, /bin and /lib are symlinks into /usr. Only inline symlink
// targets (≤ smallSymlinkSize — which covers every directory symlink in
// practice) can be followed; longer targets, loops (kernel-style
// 40-link bound) and ".." escapes resolve to nil.
func (w *Writer) findPath(root *inode, p string) *inode {
	const maxSymlinks = 40
	links := 0
	node := root
	for node != nil && len(p) != 0 {
		...
		case "", ".":
			...
			continue
		case "..":
			// inodes carry no parent pointer; ".." can only arrive via
			// a symlink target, which never escapes upward in practice.
			return nil
		}
		if !node.IsDir() {
			return nil
		...
		if child != nil && child.FileType() == format.S_IFLNK {
			links++
			target := string(child.Data)
			if links > maxSymlinks || child.Size > smallSymlinkSize || target == "" {
				return nil
			}
			if target[0] == '/' {
				node = w.root()
			}
			// Re-walk the link target followed by the rest of the
			// path, relative to the symlink's containing directory
			// (or the root for absolute targets).
			p = path.Clean(strings.TrimPrefix(target, "/") + "/" + rest)
			continue
		}
		node = child
		p = rest
...
func (w *Writer) MakeParents(name string) error {
	...
	for parentDirs != "" {
		...
		// Descend through the resolved inode — the existing child may be
		// a symlink (merged-usr /sbin → usr/sbin), and creating children
		// under the symlink inode itself would corrupt the tree.
		next := w.findPath(root, dirname)
		if next == nil || !next.IsDir() {
			return fmt.Errorf("%s: parent %s is not a resolvable directory", name, currentPath)
		}
		root = next
	}
	return nil
...
// skipToBlock advances the write position to the start of the given block,
// punching a sparse hole via Truncate when the underlying seeker supports it
// (an *os.File does) and falling back to writing literal zeros otherwise.
// The skipped region must never carry meaningful bytes — callers use it only
// for the free-block tail of a padded filesystem. (Local addition, not in
// upstream hcsshim.)
func (w *Writer) skipToBlock(block uint32) error {
	if w.err != nil {
		return w.err
	}
	target := int64(block) * BlockSize
	n := target - w.pos
	if n < 0 {
		w.err = fmt.Errorf("cannot skip backwards from %d to block %d", w.pos, block)
		return w.err
	}
	tr, ok := w.f.(interface{ Truncate(int64) error })
	if !ok {
		_, err := w.zero(n)
		return err
	}
	if target > w.maxDiskSize {
		w.err = exceededMaxSizeError{w.maxDiskSize}
		return w.err
	}
	...
	// Truncate (not just Seek) so the file's logical size matches the
	// filesystem's block count even though nothing is written after the
	// hole's start. A shorter image makes the kernel see a truncated
	// filesystem.
	if err := tr.Truncate(target); err != nil {
		w.err = err
		return err
	}
	if _, err := w.f.Seek(target, io.SeekStart); err != nil {
		w.err = err
		return err
	}
	w.pos = target
...
// Writable omits the RO_COMPAT_READONLY superblock flag, producing a
// filesystem the kernel will mount read-write. Upstream always sets the
// flag because LCOW layer images are immutable; a VM root filesystem is
// not. (Local addition, not in upstream hcsshim.)
func Writable(w *Writer) {
	w.writable = true
}

// TotalDiskSize pads the filesystem to at least size bytes, leaving the
// tail as free blocks the guest can allocate from. Without it the
// filesystem is sized exactly to its content and the guest cannot write
// so much as a log line. Content larger than size wins — the filesystem
// grows to fit and the padding disappears. (Local addition, not in
// upstream hcsshim.)
func TotalDiskSize(size int64) Option {
	return func(w *Writer) {
		if size <= 0 {
			return
		}
		if size > maxMaxDiskSize {
			size = maxMaxDiskSize
		}
...
// bytesPerInode is the disk-space-to-inode ratio used to size the inode
// table, matching mkfs.ext4's default (one inode per 16 KiB).
const bytesPerInode = 16 * 1024

// inodesForBlocks returns the inode count a normally-formatted ext4 of
// the given block count would carry (disk bytes / bytesPerInode),
// saturating at the 32-bit inode-number ceiling.
func inodesForBlocks(blocks uint32) uint32 {
	n := uint64(blocks) * BlockSize / bytesPerInode
	if n > 0xffffffff {
		return 0xffffffff
	}
	return uint32(n)
...
// totalDiskBlocks converts the requested padding size into a block count,
// saturating rather than overflowing the 32-bit block addressing this
// writer uses. Zero when no padding was requested.
func (w *Writer) totalDiskBlocks() uint32 {
	if w.totalDiskSize <= 0 {
		return 0
	}
	tb := (w.totalDiskSize + BlockSize - 1) / BlockSize
	if tb > 0xffffffff {
		return 0xffffffff
	}
	return uint32(tb)
...
// roCompatFeatures returns the RO_COMPAT superblock feature set. Writable
// images drop RO_COMPAT_READONLY — the flag makes the kernel refuse rw
// mounts, which is upstream's intent for immutable LCOW layers but wrong
// for a VM root filesystem. (Local addition, not in upstream hcsshim.)
func roCompatFeatures(writable bool) format.RoCompatFeature {
	f := format.RoCompatLargeFile | format.RoCompatHugeFile | format.RoCompatExtraIsize
	if !writable {
		f |= format.RoCompatReadonly
	}
	return f
}
...
func (w *Writer) Close() error {
	...
	wantBlocks := inodeTableOffset
	if tb := w.totalDiskBlocks(); tb > wantBlocks {
		// Padded image: lay out enough block groups to cover the requested
		// size, not just the content. The extra groups' blocks stay free.
		wantBlocks = tb
	} else if w.writable {
		// Content met or exceeded the requested size. A writable
		// filesystem with zero free blocks is unusable — the guest can't
		// append a line to /etc/passwd — so pad past the content:
		// content/8, at least 512 MiB, capped by the metadata
		// reservation. The padding is sparse and costs no physical disk.
		slack := wantBlocks / 8
		if minSlack := uint32(512 * 1024 * 1024 / BlockSize); slack < minSlack {
			slack = minSlack
		}
		limit := uint32(w.maxDiskSize / BlockSize)
		if wantBlocks+slack < wantBlocks || wantBlocks+slack > limit {
			wantBlocks = limit
		} else {
			wantBlocks += slack
		}
	}
	// Size the inode table to the filesystem, not just the content. A
	// pure "compact" writer allocates one inode per source file and
	// stops, which leaves a padded/grown rootfs (TotalDiskSize) with only
	// as many inodes as the image had files — an 8 GiB rootfs ends up with
	// a few tens of thousands of inodes, and file-heavy work (kernel
	...
	groups, inodesPerGroup := bestGroupCount(wantBlocks, inodeTarget)
	...
	if req := w.totalDiskBlocks(); req > 0 {
		// Never address blocks beyond the groups we laid out.
		if limit := groups * blocksPerGroup; req > limit {
			req = limit
		}
		if diskSize < req {
			diskSize = req
	...
	for g := uint32(0); g < groups; g++ {
		...
		// Pad the unused tail of the inode bitmap with ones. e2fsck
		// requires bits past inodesPerGroup to be set; upstream leaves
		// them zero, which its read-only LCOW images never had checked.
		// (Local fix, not in upstream hcsshim.)
	...
	// Extend the file to the disk size. The tail past the bitmaps is all
	// free blocks, so a hole is as good as written zeros and keeps a padded
	// image's physical footprint at its content size.
	if err := w.skipToBlock(diskSize); err != nil {
		return err
	}
	...
	sb := &format.SuperBlock{
		...
		FeatureRoCompat:    roCompatFeatures(w.writable),
