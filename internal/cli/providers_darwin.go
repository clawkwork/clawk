...
func newProvider(p config.Provider) (sandbox.Provider, error) {
	...
	case config.ProviderVZ:
		return sandbox.NewVZProvider(store), nil
	case config.ProviderFirecracker:
		return nil, fmt.Errorf("provider %q runs on Linux; this is a macOS build", p)
	default:
		return nil, fmt.Errorf("unknown provider %q (valid on macOS: vz)", p)
	}
}

// defaultProvider is the provider used for new sandboxes when neither
// --provider nor a clawk.mod selects one.
func defaultProvider() config.Provider { return config.ProviderVZ }
